...
import { useTranslation } from "react-i18next";
import {
  Heart, Shield, Star, Zap, Calendar, CheckCircle, ArrowRight, Sparkles, Award,
} from "lucide-react";
import CONFIG from "../../config/config.js"; // ✅ utilise ton fichier global
...
const NosValeurs = () => {
  ...
  // 🟢 Normalise URL comme dans Phototheque
  ...
  const fetchValeur = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Erreur lors du chargement des valeurs");

      const data = await res.json();
      const valeurs = Array.isArray(data.results) ? data.results : data;
      const firstValeur = valeurs?.[0];

      // 🔧 Normaliser l'image
      if (firstValeur?.image) {
        firstValeur.image = normalizeUrl(firstValeur.image);
      }

      setValeur(firstValeur || null);
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#0a0e27] flex items-center justify-center px-4">
        <div className="bg-red-500/10 border border-red-500/30 text-white p-6 rounded-2xl">
          <Zap className="w-6 h-6 text-red-500 mb-2" />
          <p>{t("values.error", "Une erreur est survenue :")} {error}</p>
        </div>
      </div>
    );
  }
  ...
  if (!valeur) {
    return (
      <div className="min-h-screen bg-[#0a0e27] flex items-center justify-center px-4">
        <div className="text-center py-20 bg-white/5 rounded-3xl border border-orange-500/20 px-8">
          <Heart className="w-12 h-12 text-orange-400 mx-auto mb-4" />
          <p className="text-white font-bold text-xl">
            {t("values.empty", "Aucune valeur disponible")}
          </p>
        </div>
      </div>
    );
  ...
  // ✅ Affichage
  return (
    <div className="min-h-screen bg-[#0a0e27] text-white pt-40 pb-20">
      {/* 🌈 Effets visuels */}
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-orange-500/10 blur-3xl rounded-full"></div>
      <div className="absolute top-1/3 right-1/4 w-96 h-96 bg-blue-500/10 blur-3xl rounded-full"></div>

      {/* 🧡 Titre */}
      <div className="text-center mb-12 relative">
        <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-orange-500 via-orange-600 to-orange-700 rounded-full shadow-2xl">
          <Heart className="w-10 h-10 text-white" />
        </div>
        <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-orange-400 to-white mt-6">
          {t("values.title", "Nos Valeurs")}
        </h1>
      </div>

      {/* 📸 Image principale */}
      <div className="w-[90%] mx-auto mb-10 rounded-3xl overflow-hidden border border-orange-500/30 shadow-2xl">
        {valeur.image ? (
          <img
            src={valeur.image}
          ...
          <div className="h-[60vh] bg-gradient-to-br from-orange-500/20 to-blue-500/20 flex items-center justify-center">
            <Sparkles className="w-20 h-20 text-orange-400/30" />
          </div>
        )}
      </div>
      ...
      <div className="text-center mt-12">
        <button
          onClick={fetchValeur}
          className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 px-8 py-3 rounded-full text-white font-bold shadow-lg"
        >
