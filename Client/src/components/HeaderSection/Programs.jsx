...
const Programs = () => {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        if (!response.ok) throw new Error(t("Erreur lors du chargement des programmes."));
  ...
  const handleClick = () => {
    navigate('/contacter-tamkine');  // Redirection vers /contacter-tamkine
  };


  return (
    <div className="bg-gradient-to-b from-blue-50 to-white min-h-screen">
      {/* En-tête harmonisée avec Partner */}
      <header className="bg-gradient-to-r from-[#1C1C47] to-[#12138B] text-white text-center py-4 px-4 md:py-8 lg:py-12 shadow-md">
        <div className="pt-16">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">{t('Nos Programmes')}</h1>
          <p className="max-w-2xl mx-auto text-base md:text-lg opacity-90">
            {t('Découvrez les différents programmes et initiatives de notre fondation pour maximiser notre impact collectif.')}
          </p>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-8 md:py-16 max-w-6xl">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-10">
            ...
            <span className="ml-3 text-gray-600 text-sm sm:text-base">{t("Chargement des programmes...")}</span>
          </div>
        ) : error ? (
      ...
      {/* Footer CTA - Similaire au Partner */}
      <div className="bg-white py-16 text-black">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            {t('Intéressé par nos programmes ?')}
          </h2>
          <p className="text-lg md:text-xl opacity-90 mb-8 max-w-2xl mx-auto">
            {t('Rejoignez nos initiatives et contribuez à transformer l\'éducation numérique avec la Fondation Tamkine.')}
          </p>
          <button
      onClick={handleClick}  // Ajout de l'événement onClick pour la navigation
