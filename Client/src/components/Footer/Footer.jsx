...
const Footer = () => {
  ...
  return (
    <footer className="bg-gradient-to-b from-[#1C1C47] to-[#15152E] text-white relative">
      ...
      <div className="container px-6 lg:px-20 pt-16 pb-8 mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 items-start">
          {/* Logo et slogan */}
          <div className="flex flex-col items-center md:items-start">
            <Link to="/home" className="mb-4 transition-transform hover:scale-105 duration-300">
              <img src={logo} alt="Tamkine Logo" className="w-32 h-32 object-contain drop-shadow-lg" />
            </Link>
          ...
          <div className="flex flex-col items-center md:items-start">
            <h4 className="font-bold text-lg mb-4 pb-2 border-b-2 border-blue-400 inline-block">Nos Plateformes</h4>
            <ul className="grid grid-cols-2 gap-3 w-full">
              {[
                ...
                <li key={idx} className="transition-all">
                  ...
                  >
                    <span className="text-blue-400 mr-1">›</span> {platform.name}
                  </a>
                </li>
              ))}
            </ul>
          ...
          {/* À propos */}
          <div className="flex flex-col items-center md:items-start">
            <h4 className="font-bold text-lg mb-4 pb-2 border-b-2 border-blue-400 inline-block">À Propos</h4>
            <ul className="space-y-3">
              {["Qui sommes-nous ?", "Contacter Tamkine", "Nous rejoindre"].map((item, idx) => (
                <li key={idx} className="transition-all">
                  <Link to={`/${item.toLowerCase().replace(/\s+/g, '-').replace(/[?]/g, '')}`} 
                    className="text-gray-300 hover:text-white hover:translate-x-1 transition-all duration-300 flex items-center">
                    <span className="text-blue-400 mr-1">›</span> {item}
                  </Link>
                </li>
              ))}
            </ul>
          ...
          <div className="flex flex-col items-center md:items-start">
            <h4 className="font-bold text-lg mb-4 pb-2 border-b-2 border-blue-400 inline-block">Notre Localisation</h4>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-12">
        <div className="bg-gradient-to-r from-[#2E2E60] to-[#3B3B7A] py-8 px-8 rounded-xl shadow-2xl transform hover:shadow-blue-900/20 hover:-translate-y-1 transition-all duration-300">
